...
function CircleEntity(x, y, radius, type, restitution,deceleration){
     ...
     this.y = y;
     this.radius = radius;
     this.restitution = 1; // 反発係数
     this.deceleration = 1; // ballの速さ
     ...
     this.collidedWithCircle = function (peer) { // 円と円の衝突
         ...
         if(this.type == BodyDynamic && peer.type == BodyStatic){
              ...
              var dot0 = this.velocity.dot(aNormUnit); // 法線と速度内積
              ...
              var aTang = aTangUnit.mul(aTangUnit.dot(this.velocity)); // aベクトル法線成分
              var bNorm = bNormUnit.mul(bNormUnit.dot(peer.velocity)); // bベクトル法線成分
              var bTang = bTangUnit.mul(bTangUnit.dot(peer.velocity)); // bベクトル法線成分
